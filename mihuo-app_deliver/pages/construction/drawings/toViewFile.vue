<template>
	<view class="view-drawings">
		<view class="swiper">
			<u-swiper :list="imgList" mode="rect" :height="swiperheight" @change="onChangeSwiper" img-mode="aspectFit"
				:autoplay="false" border-radius="0" bg-color="#fff" :title="true" :titleStyle="titleStyle"></u-swiper>
			<!-- <view class="image-name">{{imgList[currentFileIndex] && imgList[currentFileIndex].name}}</view> -->
		</view>
		<view class="info">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			eventChannel.on('toViewFile', (data) => {
				this.fileList = data
				...
				this.fileList.forEach(item => {
					if (item) {
						this.imgList.push({
							type: item.type,
							name: item.name,
							url: item.type == 'video' ? this.$util.thumbnailImage(item.url,
								false) : '',
							image: {
		...
		methods: {
			...
			equipment() {
				uni.getSystemInfo({
					success: (res) => {
						let height = res.windowHeight * (750 / res.windowWidth) * 0.70 //获取系统信息，可使用窗口的高度
