		...
		<view class="release">
			<form>
				<view class="bg_fff" style="margin-top: 10rpx;">
					<view class="top_nav">
						<text class="left_nav">{{inputLabel}}</text>
					</view>
					<view class="textarea_box">
						<textarea placeholder-style="color:#909199" :placeholder="placeholder" style="width: 100%;
						...
						<view class="upload" :key="imageUploadIndex">
							<image-upload-new :isAddWatermark="true" :address="waterAddress" :userInfo="waterUserInfo"
								:uploadType="uploadType" :count="9" v-model="form.appendPicFiles" />
						</view>
					</view>
				...
				<view class="bg_fff" style="margin-top: 10rpx;" v-if="activeStatus === '' ">
					<view class="top_nav">
...
<script>
	...
	export default {
		data() {
			return {
				uploadType: ['图片', '视频'],
				buttonList:[{name:'确认',status:true}],
				chooseVoiceObject: {},
				fileObject: {},
				conValue: '',
				option: {},
		...
		methods: {
			...
			setDataInitial() {
				...
				this.activeStatus = tempParameter.activeStatus
				
				// 选项是通过时 ，1.整改方案隐藏 2.存在问题修改为补充说明
				if(tempParameter.activeStatus){
					this.inputLabel = '补充问题'
					this.placeholder = '请输入补充问题'
				}
			...
			submit() {
				if (!this.form.appendQues && !this.activeStatus) {
					this.$util.toast(`请填写${this.activeStatus ? '补充':'存在的'}问题`);
					return
				}
