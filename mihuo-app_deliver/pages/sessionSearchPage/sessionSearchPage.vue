	...
	<view class="container">
		<z-nav-bar backState="2000">
			<view class="tui-searchbox" slot="left" style="width: 95%;padding:0 3% 20rpx 4%;">
				<view class="tui-search-input">
					...
					<input confirm-type="search" placeholder="搜索" :focus="isFocus"
		...
		<view class="tui-search-history">
			<view class="section" v-if="cancat.length">
				...
				<session-item @click.native.stop="toDetail(item)" :searchText="key" v-for="(item,index) in cancat"
					:isSearch="true" :sessionData="item" :key="item.TargetId">
				</session-item>
			</view>
			<view class="section" v-if="group.length">
				<view class="titletype"><text class="title">群聊</text></view>
				<session-item @click.native.stop="toDetail(item)" :searchText="key" v-for="(item,index) in group"
					:isSearch="true" :sessionData="item" :key="item.TargetId">
				</session-item>
			</view>
			<!-- <view class="section" v-if="searchList.length">
				<view class="titletype"><text class="title">聊天记录</text></view>
				<session-item v-for="(item,index) in searchList" :isSearch="true" :sessionData="item"></session-item>
			</view> -->
		</view>
	</view>
</template>
		...
		methods: {
			...
			async inputKey(e) {
				...
				this.searchList = this.searchList.map(item => ({
					...item,
					SearchTargetName: item.TargetName.replace(regExp,
						`<text style="color:#ef4a4a;">${this.key}</text>`)
				}))
			...
			toDetail(item) {
				...
				if (item.Type == 2) {
					...
					this.toPage(`/pages/message/chat/index?toUserId=${TargetId}&toUserName=${TargetName || 'vaonima'}`)
				}
			}
		}
	}
