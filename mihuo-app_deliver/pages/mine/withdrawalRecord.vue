<template>
	<view>
		...
		<z-nav-bar fontColor="white" :bgColor="[$styleColor.themeColor,$styleColor.themeColor]" title="">
			<view class="nav-bar" slot="default">提现记录</view>
		</z-nav-bar>
		...
		<view class="list_title">
			...
			<!-- <view class="income_expense">提现 ￥15500</view> -->
			<view style="clear:both;"></view>
		</view>
		<view class="text_list" v-if="withdrawalList.length">
			<scroll-view class="scroll-con" scroll-y @scrolltolower="onRefresh">
				<view class="order_list" v-for="item in withdrawalList" :key="item.id">
					...
					<view class="order_list_right2">-{{item.amount/100}}</view>
				</view>
			</scroll-view>
		</view>
		<mh-empty v-if="withdrawalList.length === 0" icon="/static/images/no_recommend.png" title="还没有提现记录～～" />
...
<script>
	...
	export default {
		...
		methods: {
			onRefresh() {
				if (this.withdrawalList.length == this.total) return
				this.getWithdrawalRecord()
			},
			getWithdrawalRecord() {
				this.$httpApi.withdrawalRecord({
					...
					if (res) {
						...
						this.withdrawalList = this.current > 1 ? this.withdrawalList.concat(res.records) : res
							.records
						this.current += 1
					}
