<template>
	<view class="container" @tap="togglePicker(0)">
					...
					<view v-else class="item flex-row" :class="[item.source == userId ? 'right' : 'left']">
						...
						<view v-if="item.msgType == 'map'" class="content map" @tap="openLocation(item.content)">
							...
							<map class="box"
								...
								:markers="[{latitude: item.content.latitude, longitude: item.content.longitude, iconPath: '../../../static/images/icon-address/address-5.png'}]" 
								@tap="openLocation(item.content)"
							></map>
						</view>
						<!--语音-->
						<view v-if="item.msgType == 'voice'" @tap="playVoice(item)" class="content">
							<image v-show="voicePlayingId != item.id" src="../../../static/images/audio.png" class="voice-icon"></image>
							<image v-show="voicePlayingId == item.id" src="../../../static/images/audio-play.gif" class="voice-icon"></image>
						</view>
					</view>
				</view>
				<view id="bottom"></view>
		...
		<view class="oper flex-row" @tap.prevent.stop="">
			<!--语音按钮切换-->
			<image v-if="isEdit" @tap="toggleEdit(false)" src="../../../static/images/voice-circle.png" class="icon"></image>
			...
			<!--附件-->
			<image @tap="togglePicker(86, 'file')" v-show="!content" src="../../../static/images/add.png" class="icon"></image>
		</view>
		<!--表情-->
		...
		<view v-show="showFile" class="file" @tap.prevent.stop="">
			<view class="list flex-row">
				<view class="item flex-column" @tap="uploadImage(['album'])">
					<image src="../../../static/images/album.png" mode="widthFix" class="icon"></image>
					<view class="text">相册</view>
				</view>
				<view class="item flex-column" @tap="uploadImage(['camera'])">
					<image src="../../../static/images/photo.png" mode="widthFix" class="icon"></image>
					<view class="text">拍照</view>
				</view>
				<view class="item flex-column" @tap="getLocation">
					<image src="../../../static/images/icon-address/address-2.png" mode="widthFix" class="icon"></image>
