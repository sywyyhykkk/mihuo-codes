	...
	<view>
		<view class="head_info display-center">
			<view class="left display-left">
				<view class="type">{{orderInfo.priceWay===1?'一口价':orderInfo.priceWay===2?
					'竞价模式':orderInfo.priceWay===3?'快速呼叫':''}}</view>
				<view class="type">{{orderInfo.skillName}}</view>
			</view>
			<view class="right display-left" @click="goEditOrder" v-if="orderInfo.orderStatus===10">
				<image class="edit" src="/static/images/order_details/edit.png" />
				<view class="text">编辑</view>
			</view>
		</view>
		<view class="des_title">{{orderInfo.requirementDesc}}</view>
		<view class="address_view display-left">
			<image class="address_icon" src="/static/images/order_details/address_solid.png" />
			<view class="address">
		...
		<view class="price_view" v-if="orderInfo.orderStatus!==10&&orderInfo.orderStatus!==-1">
			...
			<view @click="seeChangeData" v-if="newChangeData.alterationMoney">
				...
				<text class="refund"
					:class="newChangeData.status==0?'add_item':''">{{newChangeData.alterationMoney}}</text>元
				<image class="arrow_right" src="/static/icon/arrow_right.png" />
			</view>
			<!-- 无变更 -->
			<view v-else>
				<text class="price">{{priceState}}</text>元/
				{{orderInfo.priceWay===1?'一口价':orderInfo.priceWay===2?'师傅报价':orderInfo.priceWay===3?'快速呼叫':''}}
			</view>
		</view>
		<!-- 没确认的 -->
		...
		<view v-if="isOpen">
			...
			<view class="demand_des">
				...
				<view class="image_show" v-if="orderInfo.attachments">
					<image-show :imgSrc="orderInfo.attachments" />
				</view>
				<mihuo-audio v-if="audioObject" :formDate="audioObject" />
			</view>
		</view>
...
<script>
	/*	
	orderInfo.priceWay 1:一口价；2：报价；3：快速呼叫
	...
	export default {
		...
		methods: {
			...
			seeChangeData() {
				this.toPage(
					`/pages/findMasterWorker/changeItemList?id=${this.orderInfo.id}&orderCode=${this.orderInfo.orderCode}`
				)
			},
			goEditOrder() {
				this.toPage('/pages/findMasterWorker/issueOrder?type=edit&id=' + this.orderInfo.id)
			}
		}
	}
</script>
...
<style lang="scss" scoped>
	...
	.personal_info {
		border-top: 1rpx solid $fontColor80;
