...
<script>
	...
	export default {
		...
		props: {
			...
			// 当mode=time、isAllowPassedTime为false,isLater 的时候，只能从下一个月开始选择
			isStartTime: {
				type: [String, Number],
				default: '',
			},
			// 当mode=time、isAllowPassedTime为false 的时候，只能从下一天开始选择
			isLater: {
				type: Boolean,
				default: false,
			},
			// 当mode=time的时候，是否可以选择当前日期以前的日期
			isAllowPassedTime: {
				type: Boolean,
				default: true,
			},
		...
		methods: {
			...
			setMonths() {
				if (this.isAllowPassedTime) {
					// 允许选择之前的日期
					this.months = this.generateArray(1, 12);
				} else {
					// 不允许选择之前的月份
					this.months = this.generateArray(new Date().getMonth() + 1, 12);
				}
			...
			setDays() {
				...
				if (this.isAllowPassedTime) {
					...
					if (this.month != (new Date().getMonth() + 1)) {
						...
						// 购买保险时间调整
						if (this.isLater) {
							// 若未购买保险或者保险失效之后重新购买，时间只能从下一天开始
							if (this.isStartTime) {
								this.days = this.generateArray(Number(this.isStartTime) + 1, totalDays)
							} else {
								this.days = this.generateArray(new Date().getDate() + 1, totalDays)
								// 当前日期为当前月的最后一天，在isLater为true时，需要重置months、days
								if (new Date().getDate() + 1 > totalDays) {
									this.months = this.generateArray(new Date().getMonth() + 2, 12);
									this.days = this.generateArray(1, totalDays);
								}
							}
						}
			...
			change(e) {
				...
				if (this.mode == 'time') {
					...
					// isAllowPassedTime为true，切换年份时，this.months需要重新计算
					if (!this.isAllowPassedTime) {
