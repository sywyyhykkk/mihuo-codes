	...
	<view class="vk-data-goods-sku-popup" catchtouchmove="true" :class="valueCom && complete ? 'show' : 'none'"
        ...
        borderRadius: borderRadius + 'rpx ' + borderRadius + 'rpx 0 0'
			...
			<view class="specification-wrapper">
                ...
                " :style="{ backgroundColor: goodsThumbBackgroundColor }" mode="aspectFill" @click="previewImage">
							</image>
						</view>
						<view class="specification-right">
							<view class="price-content" :style="{ color: themeColorFn('priceColor') }">
								<text class="sign">¥</text>
								<text v-if="!isGroupProduct" class="price"
									:class="priceCom.length > 16 ? 'price2' : ''">
									<!-- 正常商品的价格 -->
									{{ priceCom }}
								</text>
								<text v-else class="price" :class="groupPrice.length > 16 ? 'price2' : ''">
									<!-- 团购商品的价格 -->
									{{ groupPrice }}
								</text>
								<text class="group-tag" v-if="isGroupProduct && isShowGroupPriceTag">团购价</text>
							</view>
							<view class="inventory" v-if="!hideStock && !isGroupProduct">{{ stockText }}：{{ stockCom }}
							</view>
							<view class="inventory" v-if="!hideStock && isGroupProduct">{{ groupStock }}</view>
                    ...
                    subIndex[index1] == index2
                      ...
                      : ''
						...
						<view class="number-box-view" v-if="isShowNumberBox">
							<view style="flex: 1">数量</view>
							<view style="flex: 4; text-align: right">
								<u-number-box size="32" v-model="selectNum" :min="minBuyNum || 1" :max="maxBuyNumCom"
									:step="stepBuyNum || 1" :positive-integer="true" @change="numChange"
									input-width="200"></u-number-box>
							</view>
						</view>
					</view>
				</scroll-view>
			...
			<view class="btn-wrapper" v-if="outFoStock || mode == 5">
          ...
          noStockText
			...
			<view class="btn-wrapper" v-else-if="mode == 4">
				<view class="sure" :style="{
			  color: themeColorFn('buyNowColor'),
			  backgroundColor: themeColorFn('buyNowBackgroundColor')
			}" @click="buyNow">
...
<script>
	...
	export default {
		...
		props: {
			...
			// 模式 1:都显示  2:只显示购物车 3:只显示立即购买 4:显示拼单购买 5:显示缺货按钮 默认 1
			mode: {
				Type: Number,
				default: 1
			},
		...
		methods: {
			...
			checkSelectShop() {
				...
				if (that.selectArr.every((item) => item != '')) {
					...
					that.selectShop['label'] = that.convertToSkuLabel(that.selectShop)
					that.$emit('getSkuLabel', that.selectShop, that.selectNum)
				} else {
					that.selectShop = {}
				}
			},
			...
			checkSelectComplete(obj = {}) {
				...
				selectShop['label'] = that.convertToSkuLabel(selectShop)
			...
			previewImage() {
				...
				let src = selectShop.image ?
					this.$util.thumbnailImage(selectShop.image) :
					this.$util.thumbnailImage(goodsInfo[goodsThumbName])
				if (src) {
					uni.previewImage({
						urls: [src]
					})
			...
			convertToSkuLabel(selectShop) {
				var that = this
				let label = ''
				selectShop['sku_name_arr'].forEach((item, index) => {
					label += item + `${index == selectShop['sku_name_arr'].length - 1 ? '' : '/'}`
				})
				return label
			}
		},
		...
		computed: {
			...
			groupPrice() {
				// 找到对应skuId的团购
				const find = this.groupSku.find(item => item.skuId === this.selectShop._id)
				if (find) {
					return find.flashPromotionPrice
				} else {
					// 如果不是参与团购的sku 返回正常价格
			...
			groupStock() {
				...
				const find = this.groupSku.find(item => item.skuId === this.selectShop._id)
				if (find) {
					return '团购库存：' + find.flashPromotionStock
				} else {
					// 如果不是参与团购的sku 返回正常库存
					return this.stockText + '：' + this.stockCom
				}
			},
			// 是否显示团购价格标签
			isShowGroupPriceTag(){
				const findIndex = this.groupSku.findIndex(item => item.skuId === this.selectShop._id)
				return findIndex !== -1
			},
			// 库存显示
			stockCom() {
