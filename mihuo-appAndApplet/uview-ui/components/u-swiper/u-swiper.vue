<template>
	<view class="u-swiper-wrap" :style="{
		...
		<swiper duration='100' acceleration :current="elCurrent" @change="change" @animationfinish="animationfinish"
			:interval="interval" :circular="circular" :duration="duration" :autoplay="autoplay"
			:previous-margin="effect3d && between ? effect3dPreviousMargin + 'rpx' : '0'"
			:next-margin="effect3d && between ? effect3dPreviousMargin + 'rpx' : '0'" :style="{
				height: height + 'rpx',
			...
			<swiper-item class="u-swiper-item" v-for="(item, index) in list" :key="index">
				<view class="u-list-image-wrap" @tap.stop.prevent="listClick(index)"
					:class="[uCurrent != index ? 'u-list-scale' : '']" :style="{
						...
						margin: effect3d && uCurrent != index ? `0 ${slideBetween}rpx` : 0,
					}">
					<!-- #ifdef MP-->
					<image :mode="imgMode"
						v-if="(item[name] || item).split('.')[(item[name] || item).split('.').length - 1] != 'mp4'"
						class="u-swiper-image" :src="item[name] || item" />
					<video :ref="'videoRef' + index" :show-fullscreen-btn="false" :enable-progress-gesture="false" :autoplay="false" v-else :controls="true" style="width: 100vw; height:960rpx;"
						:src="item[name] || item"></video>
					...
					<image :mode="imgMode"
						v-if="(item[name] || item).split('.')[(item[name] || item).split('.').length - 1] != 'mp4'"
						class="u-swiper-image" :src="item[name] || item" />
					<video ref="videoRef" :show-fullscreen-btn="false" :enable-progress-gesture="false" :autoplay="false" v-else :controls="true" style="width: 100vw; height:960rpx;"
						:src="item[name] || item"></video>
					<view v-if="title && item.title" class="u-swiper-title u-line-1" :style="[{
							'padding-bottom': titlePaddingBottom
						}, titleStyle]">
					...
					<!-- #endif -->
					<!-- #ifdef APP-PLUS -->
					<img-cache-fun ref="imgCacheRef" :mode="imgMode" class="u-swiper-image" :src="item[name] || item" />
					<view v-if="title && item.title" class="u-swiper-title u-line-1" :style="[{
							'padding-bottom': titlePaddingBottom
						}, titleStyle]">
						{{ item.title }}
		...
		<view v-if="showIndicator" class="u-swiper-indicator" :style="{
...
<script>
	...
	export default {
		...
		methods: {
			pasueVideo(){
				// #ifdef APP-PLUS
				for(const item of this.$refs.imgCacheRef){
					item.pasueVideo && item.pasueVideo()
				}
				// #endif
				// #ifndef APP-PLUS
				for(const item of this.$refs.videoRef){
