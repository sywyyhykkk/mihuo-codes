<template>
	<view class="u-load-more-wrap" :style="{
		...
		<u-line v-if="isShowLine" color="#d4d4d4" length="50"></u-line>
		<!-- 加载中和没有更多的状态才显示两边的横线 -->
		<view :class="status == 'loadmore' || status == 'nomore' ? 'u-more' : ''" class="u-load-more-inner">
			...
			<view class="u-loadmore-icon-wrap">
				<image v-if="customIcon" src="https://www.51mihuo.com/static/icon/home_v3/nomore.png" style="width: 22rpx;height: 22rpx;margin-right: 4rpx;"></image>
			</view>
			<!-- 如果没有更多的状态下，显示内容为dot（粗点），加载特定样式 -->
			<view class="u-line-1" style="padding-top: 1rpx;" :style="[loadTextStyle]" :class="[(status == 'nomore' && isDot == true) ? 'u-dot-text' : 'u-more-text']" @tap="loadMore">
				{{ showText }}
			</view>
		</view>
		<u-line v-if="isShowLine" color="#d4d4d4" length="50"></u-line>
...
<script>
	...
	export default {
		name: "u-loadmore",
		props: {
			// 加载中时是否自定义显示图标（默认false）
			customIcon: {
				type: Boolean,
				default: false
			},
			// 加载更多或没有更多时是否显示文字两边的横线
			isShowLine: {
				type: Boolean,
				default: true
			},
