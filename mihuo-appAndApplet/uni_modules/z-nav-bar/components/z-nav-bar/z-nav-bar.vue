<template>
	<view v-if="showTouTiao || touTiaoHearder">
		<view
			:class="[navFixed ? 'header_fixed' : '', type == 'transparent' ? 'header_absolute': '', navShadow ? 'header_shadow': '', themeBgColorName]"
			:style="[navBgColor ? { backgroundImage: navBgColor } : {}, { paddingTop: statusBarHeight + 'px',
					...
					<slot name="left">
						<view class="header_left_info"
							...
							<view class="header_left_back" :class="[ isTwoBtn ? 'header_btnMongol_left_back' : '' ]"
								...
								<image class="header_icon" v-else
									src="../../static/icon_back_black.png" mode="aspectFit"></image>
							</view>
							...
							<view class="header_left_home" :class="[isTwoBtn ? 'header_btnMongol_left_home' : '']"
								...
								<image class="header_icon" v-else
									src="../../static/icon_home_black.png" mode="aspectFit"></image>
							</view>
						</view>
					</slot>
		...
		<view class="header_transparentFixed header_fixed" v-if="type == 'transparentFixed'" :style="{ paddingTop: statusBarHeight + 'px',
			color: navTransparentFixedFontColor, opacity: 1 - transparentValue, zIndex: transparentValue < 0.3 ? 100 : 90}">
			<view class="header_content">
				<view class="header_left_box">
...
<script>
	...
	export default {
		...
		data() {
			// #ifdef MP-TOUTIAO
			const NotTouTiao = false;
			// #endif
			// #ifndef MP-TOUTIAO
			const NotTouTiao = true;
			// #endif
			return {
				touTiaoHearder:NotTouTiao,
				//当前页面是否是第一个页面
				firstPage: false,
				//透明度值
				transparentValue: 1,
