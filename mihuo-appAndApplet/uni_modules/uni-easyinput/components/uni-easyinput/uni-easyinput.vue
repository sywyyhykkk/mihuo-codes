<template>
	<view class="uni-easyinput" :class="{'uni-easyinput-error':msg}"
		...
		<view class="uni-easyinput__content"
			...
			:style="{'border-color':inputBorder && msg?'#dd524d':styles.borderColor,'background-color':disabled?styles.disableColor:'', 'border-style': inputBorder && msg?'solid':styles.borderStyle}">
			...
			<textarea v-if="type === 'textarea'" class="uni-easyinput__content-textarea"
				...
				@focus="onFocus" :cursor-spacing="cursorSpacing" :disable-default-padding="true" @confirm="onConfirm"
				:style="{
			...
			<input v-else :type="type === 'password'?'text':type" class="uni-easyinput__content-input" :style="{
				 ...
				 'font-size': fontSize
			 }" :cursor-spacing="cursorSpacing" :show-confirm-bar="false" :name="name" :value="val"
...
<script>
	 ...
	 * @property {Boolean}	textRight	input 文字对齐
	 * @value both	去除两端空格
	 * @value left	去除左侧空格
	 * @value right	去除右侧空格
	 * @value start	去除左侧空格
