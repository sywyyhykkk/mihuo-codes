<template>
	<view class="message-tabs">
		<view class="message-tabs-wrap">
			...
			:style="{
				...
				<slot v-bind:item="item">
					<text :style="{color: index === currentIndex ? activeColor : defaultColor}"
						class="item-title" :class="index===1&&isWorker ? 'dot-show' : ''">{{item}}
					</text>
				</slot>
			</view>
		</view>
		<view class="message-tabs-bar" :style="{
			...
			<view class="message-bottom-line" :style="{height: lineHeight,width: lineWidth}" />
		</view>
	</view>
</template>
	...
	.message-tabs-bar {
	  ...
	  transition-duration: 0.1s;
		.message-bottom-line {
			...
			height: 10upx;
			border-radius: 10rpx;
			background: linear-gradient(90deg, #FE6E32 1.72%, rgba(255, 255, 255, 0) 97.41%);
		}
	}
}
