<template>
	<view class="image-show-contain" @click.stop="">
		<view class="item-image-all" v-for="(item, index) in fileList" :style="{ marginRight: right + 'rpx' }"
			:key="index">
			<image class="item-image" v-if="item.url && (item.type === 'img' || item.type === 'video')"
				@click="preview(index)" mode="aspectFill" border-radius="10"
			...
			<image class="item-image" v-if="item.type === 'doc'" style="border-radius: 10rpx"
				@click="preview(index)" mode="aspectFill"
				:src="require(docIcon)" />
		...
		methods: {
			...
			getDealImage(val) {
				...
				if (tempList.length > 5) {
					this.fileList = tempList.slice(0, 5);
				} else {
				...
				this.fileListNum = tempList.length - 5; // 产品说这里显示的不是总数量
			},
		},
	};
</script>
