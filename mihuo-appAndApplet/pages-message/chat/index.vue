	...
	<view class="container" @tap="togglePicker(0)" @touchmove.stop.prevent="">
		<view class="scroll">
			<scroll-view scroll-anchoring upper-threshold="5" @scrolltoupper="scrpllTop"
				:scroll-into-view="scrollViewId" :scroll-y="isScroll" :style="{ height: scrollHeight }">
					...
					<view v-else class="item flex-row" :class="[item.SourceId == toUserId ? 'left' : 'right']">
						...
						<view class="messageItem" @longtap="(e)=>messageLongPress(e,item)">
							...
							<view v-if="item.Type == xsoftmsdk.enums.ContentType.Link">
								...
								<message-mh-im-billorder-item v-else-if="item.CustomType == customeType.supplierOrder"
									:materialDetail="getJSON(item.CustomContent)" />
		...
		<!--订单链接发送-->
		<!-- <message-mh-im-material-billorder v-model="linkShow" v-if="sourceType" :materialDetail="materialDetail"
		...
		<view class="oper flex-row" @tap.prevent.stop="">
			...
			<!--表情-->
			<image @tap.prevent.stop="togglePicker(200, 'emoji')" src="@/pages-message/static/images/emoji.png"
				class="icon"></image>
			<!--发送-->
			<view @touchend.prevent="send" v-show="content" class="btn">发送</view>
			<!--附件-->
			<image v-if="!content" @tap.prevent.stop="togglePicker(100, 'file')"
				src="@/pages-message/static/images/add.png" class="icon">
			</image>
		</view>
		<!--表情-->
		...
		<view v-show="showFile" class="file" @tap.prevent.stop="">
			<view class="list flex-row">
				...
				<view class="item flex-column" v-if="sourceType" @tap="showLink">
		...
		methods: {
			...
			async playVoice(item) {
				if (this.voicePlayingId) {
					...
					let furl = !item.FileId ?
						this.getJSON(item.Content).LocalPath :
					...
					console.log(this.voicePlayingId)
					this.AUDIO.src = furl;
					this.AUDIO.play();
				}
			},
			...
			retransmission(message) {
				...
				const temporaryMessage =
					xsoftmsdk.messages.NormalMessage.fromMap(remessage);
