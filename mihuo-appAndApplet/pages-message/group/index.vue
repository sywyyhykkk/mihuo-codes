<strong></strong><template>
	<view class="container" @tap="togglePicker(0)" @touchmove.stop.prevent="">
			...
			<scroll-view scroll-anchoring upper-threshold="5" @scrolltoupper="scrpllTop"
				...
				<view v-for="(item, index) in store.groupList" :key="item.KeyID">
					...
					<view v-else class="item flex-row"
						...
						<!--处理头像-->
						<message-mh-im-avatar :imId="item.SourceId" class="face" />
						<!--消息操作-->
						...
						<view>
							...
							<view class="messageItem" @longtap="messageLongPress(item)">
								...
								<view v-if="item.Type == xsoftmsdk.enums.ContentType.Image">
									<view style="position: relative;">
										<view style="position: relative">
											...
											<message-mh-img-cache @click="()=>preview(item)" v-else mode="heightFix"
								...
								<view
									...
									<image mode="aspectFill" class="box"
										src="@/pages-message/static/images/address_icon.png"></image>
									<!-- <map class="box" :latitude="getJSON(item.CustomContent).latitude" :longitude="getJSON(item.CustomContent).longitude"
										:markers="[{latitude: getJSON(item.CustomContent).latitude, longitude: getJSON(item.CustomContent).longitude, iconPath: '@/pages-message/static/images/marker.png'}]"
								...
								<view v-if="item.Type == xsoftmsdk.enums.ContentType.Voice && item.SourceId != toUserId"
									...
									<image v-show="voicePlayingId != item.CustomContent"
										src="@/pages-message/static/images/audio-play_white.png" class="voice-icon">
									</image>
									<image v-show="voicePlayingId == item.CustomContent"
										src="@/pages-message/static/images/audio-play_white.gif" class="voice-icon">
									</image>
								</view>
								<!--别人发出来的语音-->
								<view v-if="item.Type == xsoftmsdk.enums.ContentType.Voice && item.SourceId == toUserId"
									...
									<image v-show="voicePlayingId != item.CustomContent"
										src="@/pages-message/static/images/audio.png" class="voice-icon">
									</image>
									<image v-show="voicePlayingId == item.CustomContent"
										src="@/pages-message/static/images/audio-play.gif" class="voice-icon">
									</image>
								</view>
								<!--链接-->
								<view v-if="item.Type == xsoftmsdk.enums.ContentType.Link">
									<!-- 如果数据里有orderSn字段,说明是材料订单, 负责是普通订单 -->
									<message-mh-im-material-item v-if="item.CustomType == customeType.supplierMaterial"
										:materialDetail="getJSON(item.CustomContent)" />
									<message-mh-im-billorder-item
										v-else-if="item.CustomType == customeType.supplierOrder"
										:materialDetail="getJSON(item.CustomContent)" />
									<message-mh-order-info v-else-if="item.CustomType == customeType.constructionOrder"
										:orderInfo="getJSON(item.CustomContent)"
										style="background: #fff;padding: 20rpx;border-radius: 20rpx;" />
									<message-mh-order-card-content v-else :orderDetail="getJSON(item.CustomContent)" />
								</view>
								<!--电话号码的交换 卡片 等自定义消息-->
		...
		<view v-if="sourceType && orderId ">
			<message-mh-im-material-billorder v-model="linkShow" :materialDetail="materialDetail"
				:orderDetail="orderDetail" :constructionOrder="constructionOrder">
				<view slot="actionButton" class="actionButton" v-if="sourceType && orderId">
					<button class="sure cu-btn" @tap="sendLink">发送</button>
				</view>
			</message-mh-im-material-billorder>
		</view>
		<!--输入-->
		<view class="oper flex-row" @tap.prevent.stop="">
			<!--语音切换-->
			<image v-if="isEdit" @tap="toggleEdit(false)" src="@/pages-message/static/images/voice-circle.png"
				class="icon">
			</image>
			...
			<!--附件-->
			<image v-if="!content" @tap="togglePicker(100, 'file')" src="@/pages-message/static/images/add.png"
				class="icon">
			</image>
		</view>
		<!--表情-->
			...
			<view class="list">
				<!-- <view class="action">
					<view class="delete" @touchend.prevent="clearMessage">
						<image src="@/pages-message/static/images/clear-icon.png"></image>
					</view>
		...
		<view v-show="showFile" class="file" @tap.prevent.stop="">
			<view class="list flex-row">
				<view class="item flex-column" @tap="uploadImage(['camera'])">
					<image src="@/pages-message/static/images/im-takephoto.png" class="icon"></image>
					<view class="text">拍照</view>
				</view>
				<view class="item flex-column" @tap="showLink" v-if="sourceType && orderId">
					<image src="@/pages-message/static/images/im-link.png" class="icon"></image>
					<view class="text">链接</view>
				</view>
				<view class="item flex-column" @tap="uploadImage(['album'])">
					<image src="@/pages-message/static/images/im-photo.png" class="icon"></image>
					<view class="text">相册</view>
				</view>
				<!-- <view class="item flex-column">
					<image src="@/pages-message/static/images/im-file.png" class="icon"></image>
					<view class="text">文件</view>
				</view> -->
				<view class="item flex-column" @tap="getLocation">
					<image src="@/pages-message/static/images/im-address.png" mode="widthFix" class="icon"></image>
...
<script>
	...
	import {
		resolveFile
	} from "@/pages-message/components/message-mh-img-cache/index.js";
	import {
		tabSqls
	...
	export default {
		...
		methods: {
			...
			async getMaterialDetail(commodityId) {
				...
				const userId = uni.getStorageSync('userData') ? uni.getStorageSync('userData').sysUser.userId : ''
			...
			//滚到顶部加载数据
			scrpllTop() {
				const oldList = this.$store.state.groupList;
				this.$store.commit("WEBSOCKET_LOADGROUP", {
			...
			setScrollHeight(descHeight = 0) {
				// 这里不能用100vh 安卓手机会顶起来输入框
				// #ifdef MP
				this.scrollHeight = `calc(${this.deviceInfo.windowHeight}px - 150rpx - ${descHeight}px)`;
				// #endif
				// #ifdef APP-PLUS
				if (uni.getSystemInfoSync().platform == "android") {
					this.scrollHeight = `calc(${this.deviceInfo.windowHeight}px - 150rpx - ${descHeight <= 0 ? descHeight : descHeight}px)`;
				} else {
					this.scrollHeight = `calc(${this.deviceInfo.windowHeight}px - 150rpx - ${descHeight}px)`;
				}
				// #endif
			...
			async playVoice(item) {
				if (this.voicePlayingId) {
					...
					// #endif
					// #ifdef MP
					let furl = this.getFiles(item.FileId)
					// #endif
					console.log(furl);
						...
						uni.chooseLocation({
							...
							success: (chooseRes) => {
								const sendMessage = {
									...
									SourceId: parseInt(this.$store.state.imUserInfo
										.imId),
									CustomContent: JSON.stringify({
										GuidMini: xsoftmsdk.common
											.GuidMini(),
										contentType: 12,
										...chooseRes,
									}),
								...
								const temporaryMessage =
									xsoftmsdk.messages.GroupMessage.fromMap(
										sendMessage);
								imsdk.sendMessage(
									xsoftmsdk.messages.GroupMessage.fromMap(
										sendMessage)
			...
			send() {
				...
				imsdk.sendMessage(temporaryMessage);
				...
				return new Promise((res) => {
					...
					plus.io.resolveLocalFileSystemURL(
						...
						(entry) => {
							...
							entry.file((file) => {
								// 可通过entry对象操作图片
								console.log("压缩前图片信息:" + JSON.stringify(
									file)); //压缩前图片信息
								if (file.size > 504800) {
									//   如果大于500Kb进行压缩
