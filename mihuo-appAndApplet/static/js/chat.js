...
export const getShowDate = (stamp, hm = true) => {
	...
	if (diffDate < 2) {
		if (newDate != oldDate) {
			return moment(messagedate).format('dddd HH:mm');
		}
...
export const setLocalStorage = async (id, isThumb = true) => {
	...
	const [select] = await imgcache.select({
		url
	}) // 查询缓存是否存在
	if (select) {
...
export const sessionSort = (session,sessionData) => {
	const findData = session.filter(item => item
		.TargetId !=
		sessionData
		.TargetId);
	...
	const messageSessionList = [...setTop, ...noTop];
