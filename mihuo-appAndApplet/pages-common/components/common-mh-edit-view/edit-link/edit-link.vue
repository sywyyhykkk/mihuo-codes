<template>
	<view class="editImages flex-center">
		<!-- #ifdef APP-PLUS -->
		<common-mh-img-cache-fun mode="widthFix" class="image" :key="jsondata.listdata[0].name"
			@click.native="()=>toPageImage(jsondata.listdata[0])"
			:src="$util.thumbnailImage(jsondata.listdata[0].url,true)" />
		<!-- #endif -->
		<!-- #ifdef H5 -->
		<image mode="widthFix" class="image" v-for="item in jsondata.listdata" :key="item.url"
			@click="()=>toPageImage(item)" :src="$util.thumbnailImage(item.url,true)" />
		<!-- #endif -->
		<!-- #ifdef MP -->
		<image mode="widthFix" class="image" v-for="item in jsondata.listdata" :key="item.url"
			@click="()=>toPageImage(item)" :src="$util.thumbnailImage(item.url,true)" />
