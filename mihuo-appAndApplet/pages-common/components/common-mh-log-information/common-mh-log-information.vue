<template>
	<view class="log">
		<view class="log-item" v-for="(item,index) in logList" :key="index">
			...
			<view class="log-content" v-html="item.content.replace(/\n/g, '<br/>')" />
			...
			<view class="log-bottom">
				...
				<view class="flex-box" @click="lookEvaluate(item)">
		...
		<view class="open_btn" v-if="data.length>2">
			<view class="open_btn-is" @click="fold()">
				<text>查看全部</text>
			</view>
		</view>
		<!-- 查看评价 -->
...
<script>
	export default {
		...
		mounted() {
			...
			this.logList = this.data.length>2?this.data.slice(0,2):this.data;
		},
		methods: {
			fold(){
				this.$emit('goToPage',"交付")
			},
			preview(data,url){
				if(data && data.length){
