	...
	<view style="margin: 100px 0 0 0;">
		<view class="select-city-wrap" :style="'height:' + windowHeight">
			...
			<view class="top getHeight" id="top" style="background-color: #fff;">
				...
				<!-- <text>选择服务区</text> -->
				<image src="@/pages-common/static/images/close.png" class="iconfont close" @click="onClose()" />
			</view>
				...
				<view style="background-color: #FFFFFF;">
					<view style="padding-top: 20rpx;">
						<view class="current-city" id="current" v-if="position.address">当前/历史:
							{{ addressCity(positionCus.address) }}
						</view>
						<view class="city-item" v-if="position.address">
							<view class="city-item-view" style="display: flex;">
								...
								<image class='address-icon' src='@/pages-common/static/images/address_mini_icon.png' />
...
<script>
	...
	export default {
		...
		data() {
			return {
				positionCus: uni.getStorageSync('oldLocation') || {address:'定位未开启 定位未开启'},
				position: {},
				currentCity: {},
				...
				if (this.keyword === '') {
					...
					if (reg.test(this.keyword)) {
						...
						oriCityData.forEach(item => {
							let findData = item.data.filter(city => city.name.includes(this.keyword))
							let obj = {
								...item
							}
							if (findData.length) {
						...
						if (this.keyword.length === 1) {
							...
							oriCityData.forEach(item => {
								let findData = item.data.filter(city => city.pinyin.toLowerCase().includes(this.keyword.toLowerCase()))
								let obj = {
									...item
								}
								if (findData.length) {
		...
		methods: {
			...
			getAddress() {
				...
				getAppWxLatLon(async (res) => {
					let res1 = await posToCity({
						latitude: res.latitude,
						longitude: res.longitude,
					}, 1);
					this.curPOstions = res1;
					this.getOperator();
				})
			},
			// 右侧锚点跳转
			...
			async getOperator() {
				...
				this.$httpApi.getCityOperator().then(res => {
					...
					let dataCityLetter = [];
					res.some(v=>{
						if(v.address.indexOf(this.curPOstions.addressComponent.city) != -1 || v.address.indexOf(this.curPOstions.addressComponent.province) != -1){
							this.position = v;
							return false
						}
					})
