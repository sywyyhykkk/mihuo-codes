	...
	<view class="upload-page">
		<view class="upload-preview-contain" v-for="(item,index) in previewList" :key="index">
			<view v-if="item.type !== 'video'" class="upload-preview"
				...
				:style="{ backgroundImage: 'url(' + setFileUrl(item) + ')', width:(size + 'rpx'),height:(size + 'rpx')}">
				<view v-if="isShow"
					:class="[item.localPathStatus ? 'upload-preview-mask-default':'upload-preview-mask']"></view>
			</view>
			<view v-if="item.type === 'video'" class="upload-preview"
				:style="{ backgroundImage: 'url(' + $util.thumbnailVideo(item.url, true) + ')'}">
			</view>
			<image v-if="item.type === 'video'" src="https://www.51mihuo.com/static/images/video-start.png"
				class="video-play-img" mode="" @click="videoPlay(item.url)"></image>
			<image @click="close(item,'图片',index)" class="file-close"
				src="https://www.51mihuo.com/static/icon/work_type/close.png" mode="">
			</image>
		</view>
		<view class="camera" @click="getFlieUpload()" v-if="count > previewList.length && iconIsChange">
			<u-icon name="camera"></u-icon>
			<view class="text">拍个照片</view>
		</view>
		<view class="evaluation-file x6-flex x6-item-flex" @click="getFlieUpload()"
			v-else-if="count > previewList.length" :style="{ background: bg }">
...
<script>
	...
	export default {
		...
		// 				if(data && data.length){
		// 					data.map(v=>{
		// 						if(!v.localPathStatus){
		// 							v.localPathStatus = true;
		// 						}
		// 					})
		// 				}
		...
		methods: {
			...
			getAuthStatus() {
				...
				if (cameraAuthorized == 'authorized') {
					this.shooting()
				} else if (cameraAuthorized == 'denied') {
					if(uni.getStorageSync('allowCamera') != 0){
						uni.setStorageSync('allowCamera', 0)
						this.shooting()
					} else {
						uni.setStorageSync('allowCamera', -1)
						uni.showModal({
							title: "提示",
							content: "您未开启" + "相机" + "权限, 是否立即设置?",
							confirmText: "去设置",
							cancelText: "暂时不设置",
							confirmColor: "#FE6E32",
							success: success => {
			...
			shooting() {
				...
				uni.chooseImage({
					...
					// 		scope: 'scope.camera',
					// 		success: (res) => {
					// 			console.log(res,"唤起权限")
					// 			this.shooting()
					// 		},
					// 		fail: (err) => {
					// 			console.log(err,"唤起权限失败");
					// 		}
					// 	})
					// }
				})
