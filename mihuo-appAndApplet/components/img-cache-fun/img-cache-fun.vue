<template>
	<block>
		<image v-if="resource && islocal && isImg && !isAndroid" class="img-cache" :src="resource" :mode="mode" :lazy-load="lazyLoad"
		...
		<video ref="videoRef" class="video-content" :enable-progress-gesture="false" v-else-if="islocal && !isImg && isAndroid" :src="resource"></video>
	...
	export default {
		...
		data() {
			return {
				resource: '',
				islocal: false,
				isAndroid: uni.getSystemInfoSync().platform !== "ios"
			}
		},
		computed: {
			isImg() {
				if (this.resource.includes('.mp4') && this.isAndroid) {
					return false
				}
				return true
			},
		...
		methods: {
			// 暂停视频
			pasueVideo(){
				this.$refs.videoRef && this.$refs.videoRef.pasue()
			},
			// 初始化
			init() {
				// #ifdef APP-PLUS
