        ...
        <div class="dropdown-item">
          ...
          <div class="checkbox-group">
            <div v-for="(item,index) in state.tableData" v-if="state.tableData" :key="index" class="person-item"
              ...
              <el-checkbox v-if="props.multiple" @change="checkChange(item,0)" v-model="item.checked" name="type" />
              <el-radio v-else @change="checkChange(item,1)" v-model="item.checked" :label="true">{{ null }}
              </el-radio>
            </div>
          </div>
        </div>
...
const checkChange = (row: any, index: any) => {
  if (index === 0) {
    if (row.checked) {
      remove(row.employeeId)
  ...
  if (index === 1) {
    ...
    state.tableData.map((res: any) => {
      if (row.employeeId !== res.employeeId) {
        res.checked = false
      }
    })
...
const indexOf = (val: any) => {
  for (var i = 0; i < state.checkList.length; i++) {
    if (state.checkList[i].employeeId == val) {
      return i
    }
  }
  return -1
...
const getData = async (e: any) => {
  ...
  userPage({
    ...
    if (res) {
      props.multiple ? state.checkList.map((res2: any) => {
        res.data.records.map((res3: any) => {
          if (res3.employeeId === res2.employeeId) {
            res3.checked = true
            return
          }
        })
      ...
      !props.multiple ? res.data.records.map((res3: any) => {
        if (res3.employeeId == state.checkData.employeeId) {
          res3.checked = true
          return
        }
      }) : ''
