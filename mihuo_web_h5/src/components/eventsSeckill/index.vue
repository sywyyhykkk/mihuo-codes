    ...
    <div class="container">
        <el-card shadow="never">
            ...
            <div class="el-card-right">
                <el-button type="primary" icon="plus" v-if="$getPression('mall_smsflashpromotion_add')" @click="()=>dialogVisible=true">新增</el-button>
            </div>
        </el-card>
            ...
            <div class="container-main_main">
                <el-table
                    ...
                    <el-table-column label="活动名称" align="center" prop="title">
                        <template #default="scope" v-if="$getPression('mall_smsflashpromotion_view')">
        ...
        <el-dialog
            ...
            <el-form 
                ...
                <el-form-item>
                    <el-button style="margin-bottom:1rem" type="primary" icon="plus" size="mini" @click="()=>productVisible=true">添加商品</el-button>
                    <el-table  
                        stripe
                        ref="product"
                        ...
                        <el-table-column label="秒杀库存(件)" align="center">
                            <template #default="scope">
                                <el-input type="number" :min="0" oninput="if(value<0)value=0" v-model="scope.row.flashPromotionStock" placeholder="秒杀库存(件)"/>
                            </template>
                        </el-table-column>
                        <el-table-column label="秒杀价格" align="center"> 
                            <template #default="scope">
                                <el-input type="number" :min="0" oninput="if(value<0)value=0" v-model="scope.row.flashPromotionPrice" placeholder="秒杀价格"/>
                            </template>
                        </el-table-column>
                        <el-table-column label="每人限购" align="center"> 
                            <template #default="scope">
                                <el-input type="number" :min="0" oninput="if(value<0)value=0" v-model="scope.row.flashPromotionLimit" placeholder="每人限购"/>
                            </template>
                        </el-table-column>
                        <el-table-column label="仅剩数量" align="center"> 
                            <template #default="scope">
                                <el-input type="number" :min="0" oninput="if(value<0)value=0" v-model="scope.row.flashPromotionCount" placeholder="仅剩数量"/>
                            </template>
                        </el-table-column>
                        <el-table-column label="操作" width="70" fixed="right" align="center">
                            <template #default="scope">
                                <el-button size="small" type="text" class="danger" icon="delete" @click="delProduct(scope.row)">
                                    删除
                                </el-button>
                            </template>
                        </el-table-column>
...
const groupLink = (row:any)=>{
    push({ path: `/marketing/activitySeckillDetail/${row.id}` })
    afterEach((to, from) => {
      to.meta.title = '秒杀详情'
    })
}
