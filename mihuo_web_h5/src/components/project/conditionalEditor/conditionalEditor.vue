    ...
    <div class="edion-text">
      <div class="et-handel--input">
        <div class="input-left">
          <div>
            <el-dropdown>
              <el-button type="text" icon="plus" style="color: #67c23a;margin: 0 1rem" />
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item @click="handelClick('添加条件',0)">条件</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
            <el-button type="text" icon="edit-pen" disabled />
            <el-button
              type="text"
              disabled
              class="danger"
          ...
          <div v-for="(item,index) in conditionalData" :key="index" class="select-data">
            <div class="data-express">
            ...
            <div style="width: 10rem">
              <el-dropdown>
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item @click="handelClick('(',index+1)">(</el-dropdown-item>
                    <el-dropdown-item @click="handelClick(')',index+1)">)</el-dropdown-item>
                    <el-dropdown-item @click="handelClick('且',index+1)">且</el-dropdown-item>
                    <el-dropdown-item @click="handelClick('或',index+1)">或</el-dropdown-item>
                    <el-dropdown-item @click="handelClick('添加条件',index+1)">条件</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
              <el-button
                ...
                @click="handelClick('编辑条件', index, item)"
              />
              <el-button
                type="text"
                class="danger"
                icon="delete"
                @click="handelClick('删除条件', index, item.expressTranslate)"
              />
            </div>
          </div>
        </div>
...
// 页面的事件
const handelClick = (val: string, idx: number, data? : any) => {
  switch (val) {
    case '添加条件':
    ...
    case ')':
      ...
      break
    case '且':
      settingExpressItem(idx, ' 且 ')
      getExpress()
      break
    case '或':
      settingExpressItem(idx, ' 或 ')
      getExpress()
      break
  }
}
...
const settingExpressItem = (idx: number, val?: string) => {
  if (idx > 0 && !conditionalData.value[idx - 1].express && !conditionalData.value[idx - 1].expressTranslate) {
    dataIdx.value = idx - 1
    conditionalData.value.splice(dataIdx.value, 1, { express: val || '', expressTranslate: val || '', expressJson: {}})
  } else {
    dataIdx.value = idx
    conditionalData.value.splice(dataIdx.value, 0, { express: val || '', expressTranslate: val || '', expressJson: {}})
  }
}

// 取消条件item设置
const getClose = () => {
  if (!conditionalData.value[dataIdx.value].express && !conditionalData.value[dataIdx.value].expressTranslate) {
...
onMounted(() => {
  conditionalData.value = props.modelValue!.expressJson ? JSON.parse(props.modelValue!.expressJson) : []
