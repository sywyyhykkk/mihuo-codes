...
import { defineEmits, defineProps, onMounted, ref, reactive, inject } from 'vue'
...
const selectDataLoading = ref<Boolean>(false)
  ...
  ruleForms.value.validate((valid: any) => {
    if (valid) {
      ...
      expressData.express = `${state.form.source}_${state.form.name}${
        state.form.params?.length > 0
          ? '(' + state.form.params.toString() + ')'
          : ''
      } ${state.form.supportOperator} ${
        state.infoData.valueType !== '数值' ? '"' : ''
      }${state.form.dataValue}${state.infoData.valueType !== '数值' ? '"' : ''}`
...
const getSourceData = () => {
  ...
  }).finally(() => selectDataLoading.value = false)
}
