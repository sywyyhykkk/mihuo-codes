  ...
  <div>
    ...
    >
      ...
      <el-table-column align="center">
        <template #header>
          <el-select
            v-model="cooperationStatus"
            size="small"
            placeholder="合作意向"
            clearable
            @change="changeCooperation"
          >
            <el-option
        ...
        <template #default="{ row }">
          {{ row.cooperationStatus === 1 ? '已达成' : '未达成' }}
        </template>
      </el-table-column>
    ...
    <div class="statistics">
      <span
        >统计：{{ parseInt(state.stat.total) }}人反馈，{{
          parseInt(state.stat.cooperationNum)
        }}人反馈已达成，{{
          parseInt(state.stat.total) - parseInt(state.stat.cooperationNum)
        }}人反馈未达成</span
      >
    </div>
    <el-pagination
