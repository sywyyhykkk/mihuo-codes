    ...
    >
      <el-menu-item
        :index="
          resolvePath(
            onlyOneChild.path,
            showMenuTab
              ? `${activeTab === '/dashboard' ? '' : activeTab}/${basePath}`
              : ''
          )
        "
      ...
      >
        <item
          ...
          :titleName="onlyOneChild.meta.title || (item.meta && item.meta.title)"
          :isHasChidlren="false"
        />
        <template #title>
          <span
            v-if="onlyOneChild.meta.title == '塔台'"
            class="anticon-item"
            style="font-weight: 600"
            >{{ onlyOneChild.meta.title }}</span
          >
          <span v-else class="anticon-item">{{ onlyOneChild.meta.title }}</span>
        </template>
      </el-menu-item>
    </template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function hasOneShowingChild(
      ...
      const showingChildren: RouteRecordRaw[] = children.filter(
        (item: RouteRecordRaw) => {
