<template>
  <div ref="wrapper" class="tags-view-container">
    <el-button class="move-btn prev-btn" @click="move(-200)">
      <el-icon size="20">
        <CaretLeft />
      </el-icon>
    </el-button>
    <scroll-pane ref="scrollPane" class="tags-view-wrapper">
      <div class="link-wrapper">
        ...
        >
          ...
          <el-icon
            v-if="!tag.meta.affix"
            class="el-icon-close tag-icon-close"
            @click.prevent.stop="closeSelectedTag(tag)"
          ><Close /></el-icon>
        </router-link>
      </div>
    </scroll-pane>
    <el-button class="move-btn next-btn" @click="move(200)">
      <el-icon size="20">
        <CaretRight />
      </el-icon>
    </el-button>
...
export default defineComponent({
  ...
  setup() {
    ...
    watch(
      ...
      () => {
        const visited = tagsViewStore.visitedViews
        const lockList = wsCache.get('allLockList')
        for (const item in lockList) {
          const pathdata = lockList[item]
          const findPath = visited.find(
            (mini) => mini.fullPath == pathdata.path
          )
          if (!findPath && lockList.length) {
            LockManager.detory(lockList[item]?.name)
          }
        }
      },
