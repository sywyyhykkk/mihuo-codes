...
// @ts-ignore
function fetch({ url, method, params, data, headers, headersType, responseType, showErrorMessage, timeout = 0 }: Config = {
  ...
  return request({
    ...
    headers: {
      ...
      'Authorization': (!userInfo || url.indexOf('auth/oauth/token') > 0) ? 'Basic cGxhdGZvcm06cGxhdGZvcm0=' : userInfo.token_type + ' ' + userInfo.access_token,
      'Content-Type': type,
      'VERSION': api_version
    }
  })
