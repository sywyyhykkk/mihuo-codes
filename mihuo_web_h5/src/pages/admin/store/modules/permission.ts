      ...
      for (const rmap of businessMap) {
        for (const amap of asyncRouterMap) {
          if (amap.path === rmap.path) {
            ...
            router = { ...amap, ...{ icon: rmap.icon, name: rmap.name, meta: { title: rmap.name, icon: amap.meta.icon, ...amap.meta }}}
            router.children = []
              ...
              for (const rchild of rmap.children) {
                for (const achild of amap.children as AppRouteRecordRaw[]) {
                  if (achild.name === rchild.name) {
                    router.children.push({ ...achild, ...{ icon: rchild.icon, name: rchild.name, meta: { title: rchild.name, icon: achild.meta.icon, ...achild.meta }}})
                  }
                }
              }
            }
...
function getFilterRoutes(routes: any[]): any[] {
  ...
  for (const route of routes) {
    ...
    data.meta = Object.assign({}, route.meta || {}, { title: route.name || '' }, { icon: route.meta.icon })
    if (route.component) {
